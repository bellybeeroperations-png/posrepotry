...
export default function Register() {
  ...
  const totals = useMemo(() => {
    if (!order) return { subtotal: 0, discount: 0, service: 0, total: 0, combos_applied: [] };
    ...
    combos.forEach(c => {
      if (!c.active) return;
      const req = new Set(c.product_ids || []);
      if (req.size === 0) return;
      if ([...req].every(id => linePids.has(id))) {
    ...
    return { subtotal: sub, discount: disc, service: svc, total: net + svc, combos_applied: applied };
  }, [order, combos]);
